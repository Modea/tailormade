...
import './styles/SurveyKeyList.css';
...
import AddREDCapKey from './AddREDCapKey';
import { GraphQLResult } from 'aws-amplify/lib/API/types';

interface ApiKeyListResult {
  data: any
}
class SurveyKeyList extends React.Component<any, any> {
  ...
  public async componentDidMount() {
    const GetAPIKeys = `query GetAPIKeys {
      getREDCapAPIKeys(studyId: "${this.props.studyId}") {
        items {
          studyId
          surveyId
          apiKey
  ...
  private confirmDeleteKey = async () => {
    const DeleteAPIKey = `mutation DeleteAPIKey {
      deleteREDCapAPIKey(surveyId: "${this.state.activeSurveyId}", studyId: "${this.props.studyId}") {
        studyId
        surveyId
      }
    }`
  ...
  private saveKeyChanges = async () => {
    const UpdateKey = `mutation UpdateKey {
      updateREDCapAPIKey(input: {surveyId: "${this.state.activeSurveyId}", studyId: "${this.props.studyId}", friendlyName: "${this.state.activeSurveyName}", participantIdField: "${this.state.activeSurveyParticipantIdField}"}) {
        studyId
        surveyId
      }
    }`;
  ...
  public render() {
    ...
    return ( !this.state.isLoading &&
      <div className="SurveyKeyList">
        ...
        {this.state.apiKeys.length > 0 ? this.state.apiKeys.map((element) => <div key={uuid()} className="manage-study-key-item">
          <div className="key-item-name">
        ...
        </div>) : <div className="manage-study-no-keys">There are no survey keys for this study.</div>}
        <AddREDCapKey study={this.props.studyId} onAddKey={this.handleKeyChange} />
        <Dialog
